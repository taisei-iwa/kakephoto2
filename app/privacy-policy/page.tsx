...
const content = [
  ...
  {
    title: "第6条（アクセス解析ツールおよび広告配信ツールの利用）",
    body: "当店は、本ウェブサイトの利用状況を把握してサービスの改善に役立てるため、また広告の効果を測定するため、以下のツールを利用しています。これらのツールは Cookie 等を用いて閲覧に関する情報を収集しますが、氏名や住所など 個人を直接特定する情報は含まれません。",
    list: [
      "Google アナリティクス（Google LLC）：本ウェブサイトの利用状況の分析に使用します。Google シグナル機能を有効にしている場合、Google にログインし広告のカスタマイズを許可しているユーザーについて、年齢・性別・興味関心などを推定した統計情報を取得することがあります。収集の停止は、Google が提供するオプトアウトアドオン、または Google アカウントの広告設定から行えます。（利用規約 https://marketingplatform.google.com/about/analytics/terms/jp/ ／ プライバシーポリシー https://policies.google.com/privacy?hl=ja）",
      "Meta ピクセル（Meta Platforms, Inc.）：本ウェブサイトの閲覧および問い合わせの状況を計測し、広告の配信と効果測定に使用します。収集の停止は、Facebook および Instagram の広告設定から行えます。（プライバシーポリシー https://www.facebook.com/privacy/policy/）",
      "ブラウザの設定で Cookie を無効にすることにより、これらの情報収集を拒否することもできます。その場合、本ウェブサイトの一部の機能がご利用いただけなくなることがあります。",
    ],
  },
  {
    title: "第7条（お問い合わせ窓口）",
